...
"<div class='vis' id='v" + i + "div'>\
	...
	<label class='vislabel'>Cam Qty</label>\
	<input type='number' min='1' max='3' step='1' value='1' onclick='this.select()' id='cam" + i + "qty'/>\
...
function camChange() {
	var id = $(this).attr('id');
	var i = parseInt(id.substring(3, id.indexOf('qty')));
	var qty = parseInt($('#cam' + i + 'qty').val());
...
function fixVisList(pcID) {
	...
	$('#visLstDiv' + pcID).children().each(function(i) {
		...
		var cam = $(div.children()[7]);
		cam.attr('id', 'cam' + i + 'qty');
		
		
		var tr = $(div.children()[9]);
		tr.attr('id', 'v' + i + 'tr');
		
		var btn = $(div.children()[10]);
...
function findCamStartIndex(visStart) {
	var camIndex = settings.camera.idxStart;
	for (var i = 0; i < visStart; ++i) {
		var numCams = parseInt($('#cam' + i + 'qty').val());
		camIndex += numCams;
	}
	return camIndex;
}
...
function config(pcID) {
	...
	var camIndex = findCamStartIndex(visStart);
	for (var i = visStart; i < visStart + numVis; i++) {
		if (numCurDrv == 3 && $('#v' + i + 'typ').val() == 'ULSC') $('#v' + i + 'typ').attr('disabled', true);
		...
		var numCams = parseInt($('#cam' + i + 'qty').val());
		var cams = [];
		for (var j = 0; j < numCams; ++j) {
			cams.push(settings.camera.prefix + camIndex);
			++camIndex;
		}
		var camLabel = 'Cam=' + cams.join('/');
		//var camLabel = "Cam=" + (settings.camera.prefix + (i + settings.camera.idxStart));
